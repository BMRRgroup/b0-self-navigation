...
function reorderArr(arr::AbstractArray, sizeIn::AbstractArray, sizeOut::AbstractArray, permuteDims::AbstractArray; flatten::Bool=false)
    arr = reshape(arr, Tuple(sizeIn))
    arr = permutedims(arr, permuteDims)
    arr = reshape(arr, Tuple(sizeOut))
    if flatten
        return arr[:]
    else
        return arr
...
function correct_for_single_object(tissueMask)
    label_objects = label_components(tissueMask)
    sizes = map(i -> sum(label_objects .== i), 1:maximum(label_objects))
    helper_mask = label_objects .== argmax(sizes)
    tmp_filledMask = zeros(eltype(tissueMask), size(tissueMask))
    tmp_filledMask[helper_mask] .= tissueMask[helper_mask]
    return tmp_filledMask
end